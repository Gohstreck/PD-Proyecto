...
data Punto = V Float Float Float

instance Show Punto where
  show (V a b c) = "v " ++ show a ++ show b ++ show c

type Cara = [Punto]

data ObjFile = Obj { vertices :: [Punto], caras :: [Cara]}
...
cara xs= do
  ...
  symbol "/" --a/b/
  int        --a/b/c
  return [xs !! p1,xs !! p2, xs !!p3]

objFile :: Parser ObjFile
objFile = do
  contenido <- readFile "cactus.obj"
  creaObj $ splitOn "\n" contenido

creaObj :: [String] -> Parser ObjFile
creaObj x = return (creaObjAux [] [] x)

creaObjAux :: [Punto] -> [Cara] -> Parser ObjFile
creaObjAux puntos caras =
  do
    p1 <- punto3C
    creaObjAux (puntos ++ [p1]) caras
  <|>
  do
    c1 <- cara puntos
    creaObjAux puntos $ caras ++ [c1]
